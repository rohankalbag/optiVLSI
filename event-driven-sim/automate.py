...
files = ['benchmarks/' + i[:] for i in os.listdir('benchmarks')]

def modify_path(s):
    k = s.index("benchmarks")
    return (s[:k] + s[k+11:])

class EDSimulator(Problem):
    def get_name(self):
        return 'EDSimulator'

    def setup(self):
        options = mdict(circuit=files)
        base_cmd = 'python3 event-driven-sim.py -t $output_dir/normal --b'
        self.cases = [
            Simulation(
